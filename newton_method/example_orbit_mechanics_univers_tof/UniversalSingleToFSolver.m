function [rt_X, vt_X] = UniversalSingleToFSolver(r0_X, v0_X, deltat, x0, tol, kmax)
...
% Expression of function g(x) as a string used as argument to use in 
% Newton's Method to calculate x satisfying g(x) = t(x) - deltat = 0, 
%i.e. the root of g(x) assuming that mu = 1

str = strcat('(x^3*(', S, ') + ', str1, '/', str3, '*x^2*(', C, ') + ',...
    str2, '*x*', '(1 - x^2/', str5, '*(', S, ')))/', str3, '-', str4);

% Applying Newton's Method to obtain the x
func = str;

x = NewtonMethod(func, x0, tol, kmax);
